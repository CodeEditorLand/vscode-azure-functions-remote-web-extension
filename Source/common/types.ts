...
export interface IMicrosoftAuthentication {
	getSessions(
		scopes: string[],
		options?: { clientId?: string; forceNewSession?: boolean },
	): Promise<MicrosoftAuthenticationSession[]>;
}
...
export type OnlyMethods<T> = {
	[K in keyof T]: T[K] extends { request: object; response: object }
		? T[K]
		: never;
};
...
export type IJsonRpcMessage<T> =
	| IJsonRpcRequest<T>
	| IJsonRpcResponse<T>
	| IJsonRpcNotification<T>;
