...
import type { SshStream } from "@vs/vs-ssh";
import pRetry from "p-retry";
import type { IServerTunnel } from "./types";
...
export namespace Basis {
	export async function connectToTunnel(
		...
		const tunnelInstance = await getTunnel(
			tunnelDef,
			tunnelManagementClient,
		);
		const tunnelRelayTunnelClient = new TunnelRelayTunnelClient();
		...
		return pRetry(
			async () => {
				...
				const tunnelInstance =
					await tunnelManagementClientImpl.createTunnel(
						tunnel,
						tunnelRequestOptions,
					);
