import { TunnelServiceProperties } from "@vs/tunnels-contracts";
import {
	BasisClient,
	type IAuthenticationSession,
	type IMatchedTunnel,
...
import type { IMicrosoftAuthentication, IRouteResult } from "../common/types";
...
export default async function doRoute(
	...
	const controllerRoleUrl = createUrl(
		controllerRoleBaseUrl,
		controllerRoleEndpoint,
		controllerRolePort,
	);
	const tunnelID = await postRequest(controllerRoleUrl, controllerRoleBody);
	...
	try {
		const m = await getMatchingSessionForTunnelName({
			...
			sessions: await getMicrosoftAuthSessions(
				extra.microsoftAuthentication,
			),
			name: tunnel.name,
		});
...
const createUrl = (baseUrl: string, endpoint = "", port?: number): URL => {
...
export async function getRequest(requestUrl: string) {
	try {
		...
		console.error(
			`Failed after ${backoffOptions.numOfAttempts} attempts: ${error}`,
		);
...
export async function postRequest(requestUrl: URL, requestBody: any) {
	try {
		return await backOff(
			() => axios.post(requestUrl.toString(), requestBody),
			backoffOptions,
		);
	} catch (error) {
		console.error(
